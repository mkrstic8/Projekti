...
{
    public partial class Unos_TipStudija : System.Web.UI.Page
    {
        ...
        void PopulateGridview()
        {
            ...
            {
                sqlCon.Open();
                SqlDataAdapter sqlDa = new SqlDataAdapter("SELECT * FROM TipStudija", sqlCon);
                sqlDa.Fill(dtbl);
            }
        ...
        protected void EDnevnikVranje_RowCommand(object sender, GridViewCommandEventArgs e)
        {
            try
            {
                if (e.CommandName.Equals("AddNew"))
                {
                    ...
                    {
                        sqlCon.Open();
                        string query = "INSERT INTO TipStudija (name,status) VALUES (@name,@status)";
        ...
        protected void EDnevnikVranje_RowEditing(object sender, GridViewEditEventArgs e)
        {
            ...
            string currentIme = EDnevnikVranje.DataKeys[e.NewEditIndex]["name"].ToString();
        ...
        protected void EDnevnikVranje_RowUpdating(object sender, GridViewUpdateEventArgs e)
        {
            try
            {
                ...
                {
                    sqlCon.Open();
                    SqlCommand sqlCmd = new SqlCommand("UPDATE TipStudija set name=@name,status=@status where id=@id", sqlCon);
        ...
        protected void EDnevnikVranje_RowDeleting(object sender, GridViewDeleteEventArgs e)
        {
            try
            {
                ...
                {
                    sqlCon.Open();
                    string query = "DELETE FROM TipStudija WHERE id = @id";
                    SqlCommand sqlCmd = new SqlCommand(query, sqlCon);
                    sqlCmd.Parameters.AddWithValue("@id", Convert.ToInt32(EDnevnikVranje.DataKeys[e.RowIndex].Value.ToString()));
                    sqlCmd.ExecuteNonQuery();
                    PopulateGridview();
